...
- Pass the exact direct ACA/custom-domain origins through `uploadWebBlobCorsAllowedOrigins` so Blob CORS allows browser `PUT` uploads. For the simplified model this should be `https://<app-name>.<region>.azurecontainerapps.io` (or your custom domain). If omitted while `enableUploadWeb=true`, infra now falls back to `*` temporarily so redeploys do not wipe upload-web CORS, but tighten this back to the exact public origin as soon as it is known.
...
- Set `enableUploadWebAuth=true` only after `verdecora-upload-web-${environment}` exists in the Container Apps environment, and pass the secure `uploadWebEntraClientSecret` parameter together with `uploadWebEntraClientId`.
